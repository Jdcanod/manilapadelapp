...
import { vincularInvitadoAJugador } from "@/app/(dashboard)/club/ranking/actions";
...
export function VincularInvitadoButton({ invitadoId, invitadoNombre }: Props) {
    ...
    const [selected, setSelected] = useState<JugadorLite | null>(null);
        ...
        <Dialog open={open} onOpenChange={(v) => { setOpen(v); if (!v) reset(); }}>
            <DialogTrigger asChild>
                <Button
                    variant="outline"
                    size="sm"
                    className="h-9 gap-1.5 border-ochre/30 text-ochre-dark hover:bg-ochre/10"
                >
                    <Link2 className="w-3.5 h-3.5" />
                    Vincular a jugador registrado
                </Button>
            </DialogTrigger>
                    ...
                    <div className="space-y-3 py-2">
                        <div className="flex items-start gap-2 p-3 bg-red-500/10 border border-red-500/20 rounded-xl">
                            ...
                            <p className="text-xs text-ink-soft leading-relaxed">
                                ...
                                historial de parejas quedará a nombre de{" "}
