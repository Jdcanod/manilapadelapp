...
export function MatchActivityCard({ partido, currentUserId }: MatchActivityProps) {
    ...
    if (partido.pareja1 && partido.pareja2) {
        participantsDisplay = (
            <span>
                {partido.pareja1.nombre_pareja} <span className="text-olive/60 font-normal">vs</span> {partido.pareja2.nombre_pareja}
            </span>
        );
    } else if (partido.creador) {
        participantsDisplay = (
            <span>
                Partido organizado por <Link href={`/jugador/${partido.creador.id}`} className="hover:text-emerald-700 underline decoration-olive/40 underline-offset-4">{partido.creador.nombre}</Link>
            </span>
        );
    } else {
    ...
    return (
        <Card className="bg-paper-soft border-olive/20 shadow-xl overflow-hidden flex flex-col relative">
            <CardHeader className="pb-3 border-b border-olive/10 bg-olive/5 flex flex-row items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                    <Avatar className="w-10 h-10 border-2 border-olive/20">
                        <AvatarFallback className="bg-olive/10 text-emerald-700 font-black">
                    ...
                    <div>
                        ...
                        <p className="text-[10px] text-olive/70 font-medium">
                            {formattedDate} • {partido.lugar}
                        </p>
                    </div>
                </div>
                <Badge variant="outline" className={`shadow-sm whitespace-nowrap ${isTournament ? 'border-amber-500/30 text-amber-600 bg-amber-500/10' : 'border-emerald-500/30 text-emerald-700 bg-emerald-500/10'}`}>
            ...
            <CardContent className="pt-5 pb-4 px-5">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="space-y-1 flex-1">
                        <div className="text-sm font-bold text-olive/70 uppercase tracking-widest flex items-center gap-1.5">
                            <MapPin className="w-3.5 h-3.5" /> Nivel {partido.nivel}
                        </div>
                        
                        {partido.resultado && partido.resultado !== '0-0' && (
                            <div className="mt-3">
                                <span className="text-2xl font-black text-paper tracking-tighter bg-olive px-4 py-1.5 rounded-lg border border-olive/20 inline-block shadow-inner">
                                    {partido.resultado}
                                </span>
                            </div>
                        )}
                        ...
                        {winnerName && (
                            <div className="mt-4 flex items-center gap-2 text-sm text-emerald-700 font-bold bg-emerald-500/10 w-fit px-3 py-1 rounded-full border border-emerald-500/20">
            ...
            <CardFooter className="bg-olive/5 border-t border-olive/20 py-3 px-5 flex flex-col items-start gap-3">
                <div className="flex items-center gap-4 w-full">
                    <Button 
                        ...
                        className={`hover:bg-olive/10 px-2 h-8 ${hasLiked ? 'text-red-500 hover:text-red-600' : 'text-olive/70 hover:text-ink'}`}
                    >
                        <Heart className={`w-4 h-4 mr-1.5 ${hasLiked ? 'fill-current' : ''}`} />
                        <span className="font-bold">{likesCount}</span>
                    </Button>
                    
                    <div className="flex items-center text-olive/70 text-sm font-medium">
