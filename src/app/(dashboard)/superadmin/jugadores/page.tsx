    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-olive/20 pb-6 mb-6">
                <div>
                    <h1 className="text-3xl font-bold tracking-tight text-ink mb-1">Gestión de Rankings (ELO)</h1>
                    <p className="text-olive text-sm">Panel de asignación manual de ELO y Club para los jugadores de la plataforma.</p>
                </div>
            </div>
                    ...
                    <Table>
                        <TableHeader className="bg-paper/50">
                            <TableRow className="border-olive/20 hover:bg-paper-soft/50">
                                <TableHead className="text-ink w-[250px]">Jugador</TableHead>
                                <TableHead className="text-ink">Ciudad</TableHead>
                                <TableHead className="text-ink w-[200px]">Club Base</TableHead>
                                <TableHead className="text-ink w-[150px]">Puntos ELO</TableHead>
                                <TableHead className="text-right text-ink w-[100px]">Acción</TableHead>
                            </TableRow>
                        </TableHeader>
                        <TableBody>
...
function PlayerRow({ jugador, clubes }: { jugador: any, clubes: any[] }) {
    return (
        <TableRow className="border-olive/20 hover:bg-paper-dark/50 group">
            <TableCell>
                <div className="flex items-center gap-3">
                    <Avatar className="h-9 w-9 border border-olive/30">
                        <AvatarFallback className="bg-paper-dark text-xs text-olive">
                            {jugador.nombre.substring(0, 2).toUpperCase()}
                        </AvatarFallback>
                    </Avatar>
                    <div className="flex flex-col">
                        <span className="font-medium text-ink text-sm">{jugador.nombre}</span>
                        <span className="text-xs text-olive/70">{jugador.email}</span>
                    </div>
                </div>
            </TableCell>
            <TableCell className="text-olive capitalize">{jugador.ciudad || "No definida"}</TableCell>
            ...
            <TableCell>
                <form action={async (formData) => {
                    ...
                    <select
                        ...
                        className="bg-paper border border-olive/20 text-ink text-sm rounded-md px-2 py-1 flex-1 min-w-[120px]"
                    >
            ...
            <TableCell>
                <Input
                    ...
                    className="w-[100px] h-8 bg-paper/50 border-olive/20 text-ochre-dark font-mono font-bold"
                />
            </TableCell>
