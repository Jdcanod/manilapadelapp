        ...
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-olive/20 pb-6 mb-6">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-3xl font-bold tracking-tight text-ink mb-1">Torneos Ciudad (Master)</h1>
                        <p className="text-olive text-sm">Crea torneos generales por ciudad donde participan todos los jugadores.</p>
                    </div>
                </div>
                <Link href="/superadmin/torneos/nuevo">
                    <Button className="bg-violet-600 hover:bg-violet-700 text-ink flex items-center gap-2">
            ...
            <Card className="bg-paper-soft border-olive/20 shadow-xl overflow-hidden">
                ...
                <CardHeader>
                    <CardTitle className="text-ink text-lg">Circuitos Activos</CardTitle>
                    <CardDescription className="text-olive">Listado de todos los torneos generales creados desde este panel.</CardDescription>
                </CardHeader>
                <CardContent className="p-0">
                    <Table>
                        <TableHeader className="bg-paper/50">
                            <TableRow className="border-olive/20 hover:bg-paper-soft/50">
                                <TableHead className="text-ink">Torneo</TableHead>
                                <TableHead className="text-ink">Ciudad</TableHead>
                                <TableHead className="text-ink">Fechas</TableHead>
                                <TableHead className="text-center text-ink">Formato / Nivel</TableHead>
                                <TableHead className="text-right text-ink">Estado / Ver</TableHead>
                            </TableRow>
                        </TableHeader>
                        <TableBody>
                            {torneosList.length === 0 ? (
                                <TableRow>
                                    <TableCell colSpan={5} className="text-center py-8 text-olive/70">
                                        No hay torneos master registrados todavía.
                                    </TableCell>
                                </TableRow>
                            ) : torneosList.map((t) => (
                                <TableRow key={t.id} className="border-olive/20 hover:bg-paper-dark/50">
                                    <TableCell className="font-medium text-ink">
                                        <div className="flex items-center gap-2">
                                            {t.nombre} 
                                        </div>
                                    </TableCell>
                                    <TableCell className="text-olive">
                                        <div className="flex items-center gap-1">
                                            <MapPin className="w-3 h-3 text-olive/70" /> {t.ciudad}
                                        </div>
                                    </TableCell>
                                    <TableCell className="text-olive text-sm">
                                        <div className="flex items-center gap-1">
                                            <Calendar className="w-3 h-3 text-olive/70" />
                                    ...
                                    <TableCell className="text-right">
                                        <div className="flex justify-end gap-3 items-center">
                                            <Badge className={
                                                t.estado === 'abierto' ? 'bg-olive/20 text-olive border-olive/30' : 
                                                t.estado === 'en_curso' ? 'bg-ochre/20 text-ochre-dark border-ochre/30' : 
                                                'bg-neutral-500/20 text-olive border-olive/30'
                                            }>
                                                {t.estado === 'abierto' ? 'Abierto' : t.estado === 'en_curso' ? 'Jugando' : 'Terminado'}
                                            </Badge>
                                            <Link href={`/superadmin/torneos/${t.id}`}>
                                                <Button size="icon" variant="ghost" className="h-8 w-8 text-olive hover:text-ink hover:bg-paper-dark">
